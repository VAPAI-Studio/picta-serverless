...
# ===================== BACKGROUND RECOLORING FUNCTIONS =====================
...
def get_person_mask_rembg(img_rgb: np.ndarray) -> np.ndarray:
    ...
    mask = improve_mask_consistency(mask)
...
def process_image_recolor(img_rgb: np.ndarray,
    ...
    mask_person = get_person_mask_rembg(img_rgb)
...
def recolor_handler(job_input, job_id):
...
# ===================== END BACKGROUND RECOLORING FUNCTIONS =====================


def handler(job):
    """
    Handles a job using ComfyUI via websockets for status and image retrieval,
    or processes background recoloring based on the task type.
    ...
    if isinstance(job_input, dict) and job_input.get("task_type") == "recolor_background":
        return recolor_handler(job_input, job_id)
